...
public class Stage4Test {
	...
	public void testFirstPeak() {
		assertEquals(1, testObject.firstPeak(new int[] {10,70,20,90}));
		assertEquals(3, testObject.firstPeak(new int[] {10,20,30,40}));
		assertEquals(0, testObject.firstPeak(new int[] {40,30,20,10}));
		int[] data = new int[1024*1024];
		assertEquals(0, testObject.firstPeak(data));
		int idx = (int)(Math.random()*data.length);
		for(int i=1; i < data.length; i++) {
			data[i] = data[i-1]+1;
		}
		assertEquals(data.length-1, testObject.firstPeak(data));
		data[idx]++;
		assertEquals(idx, testObject.firstPeak(data));		
	}
	
	@Test
	public void testInverseModulo() {
		assertNotNull(testObject.inverseModule(5, 37));
		assertNotNull(testObject.inverseModule(9, 20));
		assertNotNull(testObject.inverseModule(16, 61));
		assertNotNull(testObject.inverseModule(15, 64));
		assertNotNull(testObject.inverseModule(20, 9));
		assertEquals(15, testObject.inverseModule(5, 37));
		assertEquals(9, testObject.inverseModule(9, 20));
		assertEquals(42, testObject.inverseModule(16, 61));
		assertEquals(47, testObject.inverseModule(15, 64));
		assertEquals(5, testObject.inverseModule(20, 9));
		assertNull(testObject.inverseModule(20, 180));
	}
}
