...
public class OrderingLogicTest {
	...
	public void testRemove() {
		...
		String expected = "["+before.substring(before.indexOf(",")+1);
		eng1.remove(0);
		String actual = Arrays.toString(eng1.data);
		assertEquals(9, eng1.data.length, "After removing item "+itemRemoved+" from "+before+", it should be "+expected+", but is "+actual);
		for(int i=1; i < copy.length; i++) {
			assertEquals(eng1.data[i-1], copy[i], "After removing item "+itemRemoved+" from "+before+", it should be "+expected+", but is "+actual);
		}
		before = Arrays.toString(eng1.data);
		expected = "["+before.substring(before.indexOf(",")+1);
		itemRemoved = eng1.data[eng1.data.length-1];
		eng1.remove(eng1.data.length-1);
		actual = Arrays.toString(eng1.data);
		assertEquals(8, eng1.data.length, "After removing item "+itemRemoved+" from "+before+", it should be "+expected+", but is "+actual);
		...
		eng1.remove(4);
		actual = Arrays.toString(eng1.data);
		assertEquals(7, eng1.data.length, "After removing item "+itemRemoved+" from "+before+", it should be "+expected+", but is "+actual);
		for(int i=1; i <= 4; i++) {
			assertEquals(eng1.data[i-1], copy[i], "After removing item "+itemRemoved+" from "+before+", it should be "+expected+", but is "+actual);
		}
		for(int i=6; i < eng1.data.length; i++) {
			assertEquals(eng1.data[i-2], copy[i], "After removing item "+itemRemoved+" from "+before+", it should be "+expected+", but is "+actual);
		}
	}
